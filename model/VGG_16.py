...
from general_function.read_data_VGG import DataSpeech
...
class ModelSpeech():
    ...
    def CreateModel(self):
        ...
        x = Conv2D(64, (3, 3), use_bias=False, activation='relu', padding='same', kernel_initializer='he_normal')(input_data)
        ...
        x = Conv2D(256, (3, 3), use_bias=True, activation='relu', padding='same', kernel_initializer='he_normal')(x)
        x = Conv2D(256, (3, 3), use_bias=True, activation='relu', padding='same', kernel_initializer='he_normal')(x)
        x = Conv2D(256, (3, 3), use_bias=True, activation='relu', padding='same', kernel_initializer='he_normal')(x)
        x = MaxPooling2D(pool_size=(2, 2), strides=(2, 2), padding='same')(x)
        x = Dropout(0.1)(x)

        x = Conv2D(512, (3, 3), use_bias=True, activation='relu', padding='same', kernel_initializer='he_normal')(x)
        x = Conv2D(512, (3, 3), use_bias=True, activation='relu', padding='same', kernel_initializer='he_normal')(x)
        x = Conv2D(512, (3, 3), use_bias=True, activation='relu', padding='same', kernel_initializer='he_normal')(x)
        x = MaxPooling2D(pool_size=(2, 2), strides=(2, 2), padding='same')(x)
        ...
        print(x.shape)
        #exit(0)
        #x = Reshape((50, x.shape[2]*x.shape[3]))(x)
        x = Reshape((100, 2560))(x)

        x = Dense(1024, activation='relu', use_bias=True, kernel_initializer='he_normal')(x)
        x = Dropout(0.5)(x)
        x = Dense(1024, activation='relu', use_bias=True, kernel_initializer='he_normal')(x)
        x = Dropout(0.5)(x)
    ...
    def TrainModel(self, epoch=10, save_step=1000, batch_size=32):
        ...
        self.data = DataSpeech('../feature', feature_type=FEATURE_TYPE, corpus='thchs30', type='train', audio_length=self.AUDIO_LENGTH, feature_size=self.AUDIO_FEATURE_LENGTH)
        trainGen = self.data.data_generator(batch_size)
        trainNum = self.data.getDataNum()

        dev_data = DataSpeech('../feature', feature_type=FEATURE_TYPE, corpus='thchs30', type='dev', audio_length=self.AUDIO_LENGTH, feature_size=self.AUDIO_FEATURE_LENGTH)
    ...
    def LoadModel(self, filename='model_speech/' + ModelName + '/speech_model' + ModelName + '.model'):
    ...
    def SaveModel(self, filename='model_speech/' + FEATURE_TYPE + '_' + ModelName + '/' + ModelName, comment=''):
        '''
        保存模型参数
        '''
        if not os.path.exists('model_speech/' + FEATURE_TYPE + '_' + ModelName):
            os.makedirs('model_speech/' + FEATURE_TYPE + '_' + ModelName)
        self._model.save_weights(filename + comment + '.model')
        self.base_model.save_weights(filename + comment + '.model.base')
        f = open('model_speech/' + FEATURE_TYPE + '_' + ModelName + '/step_' + ModelName + '.txt', 'w')
    ...
    def TestModel(self, datapath='../feature', dataset='dev', data_count=32, out_report=False, show_ratio=True):
        ...
        self.data = DataSpeech('../feature', FEATURE_TYPE, 'thchs30', dataset, self.AUDIO_LENGTH, self.AUDIO_FEATURE_LENGTH)
        ...
        try:
            ...
            for i in range(data_count):
                ...
                pre = self.Predict(data_input, data_input.shape[0] // 16)
